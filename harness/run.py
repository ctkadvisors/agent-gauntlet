...
def run_task(task_dir: Path, endpoint: str, model: str) -> dict:
    ...
    try:
        ...
        return {"task": task_dir.name, "passed": False, "error": str(exc)[:300]}
    ...
    (workspace / "gauntlet-transcript.json").write_text(
        json.dumps(agent.transcript, indent=2)[:400_000], encoding="utf-8")
    return {"task": task_dir.name, "passed": passed, **stats,
